...
$checkRole = function ($role) use ($app) {
    return function () use ($role, $app) {
        ...
        if ($rejected) {
            throw new BearableException('No tiene permiso para realizar esta acción, debe estar registrado', 403);
        }
    };
};
...
$app->get('/Mensajes/:idUsu', 'PortalCtrl:verMensajes')->name('shwMsjUsuario');
