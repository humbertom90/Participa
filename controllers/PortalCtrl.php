...
class PortalCtrl extends Controller {
    ...
    public function verPortal() {
         $this->render('portal/contenidos.twig', array('hola' => true));
    }

    public function verMensajes($idUsu){
        $this->render('portal/Mensajes.twig',['idUsu' => $idUsu]);
    }
    ...
    public function login() {
        ...
        if ($vdt->validate($req->post()) && $this->session->login($vdt->getData('email'), $vdt->getData('password'))) {
            header('Location: http://ga.lujan.gob.ar:8000/portal?categoria=TODOS');
            exit;
        } else {
    ...
    public function registrar() {
        ...
        $vdt->addRule('nombre', new Validate\Rule\Alpha(array(' ')))
            ...
            ->addRule('email', new Validate\Rule\MaxLength(128))
            ->addRule('direccion', new Validate\Rule\MinLength(1))
        ...
        $preuser->direccion = $vdt->getData('direccion');
        $preuser->emailed_token = bin2hex(openssl_random_pseudo_bytes(16));
        $preuser->save();
        if ($this->getMode() != 'testing') { 
			$headers = "From: Lujan Participa < ga.lujan.gov.ar >\r\n";
			$to = $preuser->email;
            $subject = 'Confirma tu registro en la plataforma Lujan Participa';
			$message = 'Hola, te registraste en Lujan Participa. Entra a este link para confirmar tu email: ' . $req->getUrl() .
    ...
    public function verificarEmail($idPre, $token) {
        ...
        if ($token == $preuser->emailed_token) {
			...
			$usuario->direccion = $preuser->direccion;
            $usuario->puntos = 0;
    ...
    public function recuperarClave() {
        ...
        if ($this->getMode() != 'testing') {
            $headers = "From: Lujan Participa < ga.lujan.gov.ar >\r\n";
			$to = $usuario->email;
            $subject = 'Lujan Participa - Reiniciar clave';
            $message = 'Hola, solicitaste reiniciar tu clave en la plataforma Lujan Participa. En caso de no haberlo hecho, ' .
                'simplemente ignora este email. Pero si realmente lo hiciste, ingresa a ' . $req->getUrl() .
